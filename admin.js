...
const olderOrdersEl = document.getElementById("admin-older-orders");
const olderOrdersBtn = document.getElementById("admin-older-orders-btn");
const ordersBottomTriggerEl = document.getElementById("admin-orders-bottom-trigger");
const sellsSectionEl = document.getElementById("section-sells");
...
async function loadOrders() {
  ...
  allAdminOrdersCache = orders;
  const currentOrders = orders.filter((order) => !isOlderSellOrder(order));
  olderAdminOrdersCache = orders.filter((order) => isOlderSellOrder(order));

  if (!orders.length) {
    ordersEl.innerHTML = '<p class="desc">No sells yet.</p>';
    if (olderOrdersEl) {
      olderOrdersEl.classList.add("hidden");
      olderOrdersEl.innerHTML = "";
    }
    if (olderOrdersBtn) {
      olderOrdersBtn.classList.add("hidden");
      olderOrdersBtn.textContent = "Older sells list";
    }
    updateOlderOrdersButtonVisibility();
    return;
  }
  ...
  if (!currentOrders.length) {
    ordersEl.innerHTML = '<p class="desc">No current sells. Scroll to bottom and open older sells list.</p>';
  } else {
    ordersEl.innerHTML = renderOrdersMarkup(currentOrders);
    bindOrderActionButtons(ordersEl);
  }

  if (olderOrdersEl) {
    olderOrdersEl.classList.add("hidden");
    olderOrdersEl.innerHTML = olderAdminOrdersCache.length
      ? renderOrdersMarkup(olderAdminOrdersCache)
      : '<p class="desc">No older sells found.</p>';
    if (olderAdminOrdersCache.length) {
      bindOrderActionButtons(olderOrdersEl);
    }
  }

  if (olderOrdersBtn) {
    olderOrdersBtn.textContent = olderAdminOrdersCache.length
      ? `Older sells list (${olderAdminOrdersCache.length})`
      : "Older sells list";
  }
...
olderOrdersBtn?.addEventListener("click", () => {
  if (!olderOrdersEl || !olderAdminOrdersCache.length) return;

  const isHidden = olderOrdersEl.classList.contains("hidden");
  if (isHidden) {
    olderOrdersEl.classList.remove("hidden");
    olderOrdersBtn.textContent = "Hide older sells list";
    olderOrdersEl.scrollIntoView({ behavior: "smooth", block: "start" });
  } else {
    olderOrdersEl.classList.add("hidden");
    olderOrdersBtn.textContent = `Older sells list (${olderAdminOrdersCache.length})`;
  }

  updateOlderOrdersButtonVisibility();
});

window.addEventListener("scroll", updateOlderOrdersButtonVisibility, { passive: true });
window.addEventListener("resize", updateOlderOrdersButtonVisibility);
...
function isOlderSellOrder(order) {
  const status = String(order?.status || "").toLowerCase();

  if (status === "delivered") {
    const deliveredAtMs = toTimeMs(order?.delivered_at) ?? toTimeMs(order?.created_at);
    return Number.isFinite(deliveredAtMs) && (Date.now() - deliveredAtMs >= ONE_DAY_MS);
  }

  if (status === "returned") {
    const returnedAtMs = toTimeMs(order?.returned_at) ?? toTimeMs(order?.created_at);
...
function updateOlderOrdersButtonVisibility() {
  if (!olderOrdersBtn) return;
  ...
  const olderListOpened = olderOrdersEl && !olderOrdersEl.classList.contains("hidden");
  if (olderListOpened) {
    olderOrdersBtn.classList.remove("hidden");
    return;
  }

  const sellsSectionVisible = sellsSectionEl && !sellsSectionEl.classList.contains("hidden");
  const reachedBottom = ordersBottomTriggerEl
    ? ordersBottomTriggerEl.getBoundingClientRect().top <= window.innerHeight
    : false;

  olderOrdersBtn.classList.toggle("hidden", !(sellsSectionVisible && reachedBottom));
}
...
function renderOrdersMarkup(orders) {
  return orders.map((o) => {
    ...
    const returnedAtMs = o.returned_at ? new Date(o.returned_at).getTime() : null;
    const deliveredLocked = String(o.status || "").toLowerCase() === "delivered"
      && Number.isFinite(deliveredAtMs)
      && (Date.now() - deliveredAtMs >= ONE_DAY_MS);
    const cancelledLocked = String(o.status || "").toLowerCase() === "cancelled"
      && Number.isFinite(cancelledAtMs)
      && (Date.now() - cancelledAtMs >= ONE_DAY_MS);
    const returnedLocked = String(o.status || "").toLowerCase() === "returned"
      && Number.isFinite(returnedAtMs)
      && (Date.now() - returnedAtMs >= ONE_DAY_MS);
    const statusLocked = deliveredLocked || cancelledLocked || returnedLocked;
    const lockMessage = deliveredLocked
      ...
      : (cancelledLocked
        ...
        : (returnedLocked
          ? "Status locked: this order was returned more than 24 hours ago."
          : ""));
...
function bindOrderActionButtons(container) {
  ...
  container.querySelectorAll(".delivery-btn").forEach((button) => {
    button.addEventListener("click", async () => {
  ...
  container.querySelectorAll(".admin-status-btn").forEach((button) => {
    button.addEventListener("click", async () => {
      const id = Number(button.dataset.id);
